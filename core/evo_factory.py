...
def evaluate(individual):
    ...
    persistence = round(max(0.55, sharpe * 0.84), 2)
    return sharpe,
...
def evolve_new_alpha():
    ...
    desc = f"SMA({int(best[0])},{int(best[1])})+RSI({int(best[2])})+Hold({int(best[3])}) – full closed-loop: ShadowCrowd Oracle + CausalForge Engine + Financial Omniverse + Liquidity Teleporter + EvoAlpha Factory"
    persistence = round(max(0.6, sharpe * 0.85), 2)
